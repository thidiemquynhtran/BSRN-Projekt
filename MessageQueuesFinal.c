...
int main() {
  ...
  if (conv_pid == 0) {
    printf("\n Conv: PID: %i, PPID: %i", getpid(), getppid());
    printf("\n Conv: PPID: %i", getppid());
    // Conv-Prozess
    while (1) {
      // Messwerte (Zufallszahlen) erzeugen
      int wert = rand() % 100;
      struct message msg;
      msg.mtype = 1; // Nachrichtentyp festlegen
      // Messwert in den Textbereich der Nachricht schreiben
      sprintf(msg.mtext, "%d", wert);
      // Messwert in die Log-Message Queue schreiben
      if (msgsnd(msgid_log, &msg, sizeof(msg.mtext), IPC_NOWAIT) == -1) {
        printf("Das Schreiben des Werts ist fehlgeschlagen.\n");
        perror("msgsnd - Conv");
        exit(1);
      }
      sleep(1); // Zeitverzögerung
      // Messwert in die Stat-Message Queue schreiben
  ...
  log_pid = fork();
  if (log_pid == 0) {
    printf("\n Log: PID: %i, PPID: %i", getpid(), getppid());
    printf("\n Log: PPID: %i", getppid());
    //  Log-Prozess
    while (1) {
      // Messwerte aus der Log-Message Queue lesen und in eine lokale Datei
      // schreiben
      struct message msg;
      int rc_msgrcv =
          msgrcv(msgid_log, &msg, sizeof(msg.mtext), 1, MSG_NOERROR);
      if (rc_msgrcv < 0) {
      ...
      FILE *file = fopen("MessageQueuesFile.txt", "a");
      if (file == NULL) {
        perror("fopen - Log");
        exit(1);
      }
      // Messwert in die Datei schreiben
      fprintf(file, "Messwert: %s\n", msg.mtext);
      fclose(file);
    }
  } else if (log_pid == -1) {
    perror("\nEs kam bei fork() zu einem Fehler!");
  ...
  if (stat_pid == 0) {
    printf("\n Stat: PID: %i, PPID: %i", getpid(), getppid());
    printf("\n Stat: PPID: %i", getppid());
    // Stat-Prozess
    int sum = 0;
    int counter = 0;
    while (1) {
      // Messwerte aus der Stat-Message Queue lesen und statistische Daten
      // berechnen
      struct message msg;
      int rc_msgrcv =
      ...
      // Messwert aus dem Textbereich der Nachricht extrahieren
      int wert = atoi(msg.mtext);
      sum += wert;
      counter++;
      float mittelwert = (float)sum / counter;
      // Ergebnisse in die Report-Message Queue schreiben
      sprintf(msg.mtext, "\n Anzahl der Werte:%d, Summe: %d, Mittelwert: %.2f",
              counter, sum, mittelwert);
      if (msgsnd(msgid_report, &msg, sizeof(msg.mtext), IPC_NOWAIT) == -1) {
  ...
  if (report_pid == 0) {
    ...
    //  Report-Prozess
    while (1) {
      // Statistikdaten aus der Report-Message Queue lesen und in der Shell
      // ausgeben
      struct message msg;
      int rc_msgrcv =
          msgrcv(msgid_report, &msg, sizeof(msg.mtext), msg.mtype, MSG_NOERROR);
      if (rc_msgrcv < 0) {
        printf("Lesen der Nachricht bei Report fehlgeschlagen.\n");
        perror("msgrcv - Report");
        exit(1);
      } else {
        printf("\nReport - Statistische Daten: %s\n", msg.mtext);
        printf("--------------------\n");
        sleep(1);
      }
    }
